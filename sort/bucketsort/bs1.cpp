...
void bucketsort(int* input)
{
    ...
    // hashnew the input and insert the content into appropriate bucket based on hashnew.
    for (int i=0;i<INPUT_SIZE;i++)
    {
        int hashnewValue = hashnew(input[i]);
        if (hashnewValue >= BUCKET_K)
            hashnewValue = BUCKET_K-1;

        buckets[hashnewValue]->push(input[i]);
    }
